function addTotal() {
  ...
  function generarTicket() {
    ...
    pdf.text(`Proveedor: ${proveedor}`, 10, 70);
...
function generarTicketTXT() {
  ...
  const empleado = document.getElementById("bienvenidaContainer").textContent.replace("Bienvenido ", "") || 'N/A';
  const fecha = obtenerFechaHoraActual();
  ...
  const filas = document.querySelectorAll("#tablaDetallesCompraContainer table tbody tr");
  filas.forEach((fila) => {
      const descripcion = fila.querySelector("td:nth-child(2)").textContent.trim().slice(0, 10); // Limitar a 10 caracteres
      ...
      contenido += `${cantidad.padEnd(5)}${descripcion.padEnd(10)}${precio.padStart(8)}${subtotal.padStart(8)}\n`;
  });
...
function generarTicketPDF() {
  const pdf = new window.jspdf.jsPDF({
      unit: "mm",
      format: [80, 297], // Tamaño típico de ticket térmico
  });
  ...
  pdf.setFontSize(6);
  pdf.text("COMPRA", 20, 15, { align: "center" });

  // Detalles del encabezado
  pdf.setFontSize(6);
  pdf.text(`Sucursal: ${nombreSucursal}`, margenIzquierdo, 25);
  pdf.text(`Dirección: ${direccionSucursal}`, margenIzquierdo, 30);
  pdf.text(`Teléfono: ${telefonoSucursal}`, margenIzquierdo, 35);
  pdf.text(`Fecha: ${fecha}`, margenIzquierdo, 40);
  pdf.text(`Proveedor: ${proveedor}`, margenIzquierdo, 45);
  pdf.text(`Encargado: ${empleado}`, margenIzquierdo, 50);
  ...
  const filas = document.querySelectorAll("#tablaDetallesCompraContainer table tbody tr");
  filas.forEach((fila) => {
      const descripcion = fila.querySelector("td:nth-child(2)").textContent.trim().slice(0, 12).padEnd(8); // Limitar a 12 caracteres
      const cantidad = fila.querySelector("td:nth-child(3)").textContent.trim().padEnd(5); // 5 espacios
      const precio = parseFloat(fila.querySelector("td:nth-child(4)").textContent.trim().replace(/,/g, ""))
          .toLocaleString()
          .padStart(8); // Reservar espacio para montos grandes
      const subtotal = parseFloat(fila.querySelector("td:nth-child(5)").textContent.trim().replace(/,/g, ""))
          .toLocaleString()
  ...
  y += 5;
  const total = parseFloat(document.getElementById("total").value || '0').toLocaleString(); // Formatear con separadores de miles
  pdf.setFontSize(8);
